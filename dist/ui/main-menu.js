...
function showHelp(mctx) {
    mctx.ui.notify("Subcommands:\n" +
        ...
        "  /msg-bridge widget                — toggle status widget", "info");
}
// ── Actions ─────────────────────────────────────────────────────────────────
async function doConnect(mctx) {
    if (!acquireLock()) {
    ...
    try {
        ...
        releaseLock();
        mctx.ui.notify(`❌ Connection failed: ${err.message}`, "error");
    }
}
async function doDisconnect(mctx) {
...
async function doConfigure(mctx) {
    const platform = await mctx.ui.select("Configure transport", ["Matrix"]);
    if (!platform)
        return;
        ...
        case "Matrix": {
            ...
            if (acquireLock()) {
                try {
                    ...
                    releaseLock();
                    mctx.ui.notify(`⚠️ Matrix setup error: ${err.message}`, "error");
                }
            }
            else {
...
export async function openMainMenu(mctx) {
    const mainMenu = async () => {
        ...
        const anyConnected = mctx.transportManager
            .getStatus()
            .some((s) => s.connected);
        const choices = [

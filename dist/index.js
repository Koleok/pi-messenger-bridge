...
export default function (pi) {
    ...
    pi.on("session_start", async (_event, context) => {
        ...
        (async () => {
            ...
            if (config.matrix?.homeserverUrl && config.matrix?.accessToken) {
                transportPromises.push(Promise.resolve().then(() => {
    ...
    pi.on("turn_start", async (_event, _context) => {
        if (pendingRemoteChat) {
            try {
                await transportManager.sendTyping(pendingRemoteChat.chatId, pendingRemoteChat.transport);
            }
            catch (_err) {
    ...
    pi.on("turn_end", async (event, _context) => {
        ...
        try {
            ...
            if (toolCallsText && !config.hideToolCalls)
            ...
            for (const chunk of chunks) {
                await transportManager.sendMessage(pendingRemoteChat.chatId, pendingRemoteChat.transport, chunk);
            }
            if (!hasPendingTools) {
            ...
            switch (subcommand) {
                ...
                case "connect":
                    ...
                    try {
                        ...
                        releaseLock();
                        context.ui.notify(`❌ Connection failed: ${err.message}`, "error");
                    }
                    break;
                case "disconnect": {
                    await transportManager.disconnectAll();
                        ...
                        case "matrix": {
                            ...
                            if (acquireLock()) {
                                try {
                                    ...
                                    releaseLock();
                                    context.ui.notify(`⚠️ Matrix setup error: ${err.message}`, "error");
                                }
                            }
                            else {
